...
public:
    ...
    ~MessageToYamlBridge()
    {
        // Release the cached Python objects while holding the GIL, as the implicit
        // member destruction would decrease the reference counts without it otherwise
        const auto gilState = PyGILState_Ensure();

        m_messageClasses.clear();
        m_deserializeMessage = PyObjectRef();
        m_getMessage = PyObjectRef();
        m_messageToYaml = PyObjectRef();
